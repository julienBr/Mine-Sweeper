...
public class Custom extends JDialog implements ActionListener {
  ...
  public Custom(Frame frame, String title, boolean mod, int height, int width, int mines) {
    super(frame, title, mod);
    H=height;
    W=width;
    M=mines;
    minesweep=(MineSweeper) frame;
    try {
      jbInit();
      pack();
    }
  ...
  private void jbInit() throws Exception {
    ...
    h.setText(""+H);
    w.setBorder(border1);
    w.setMinimumSize(new Dimension(40, 21));
    w.setPreferredSize(new Dimension(40, 21));
    w.setText(""+W);
    m.setBorder(border1);
    ...
    panel1.add(w,   new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0
            ,GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 20), 0, 0));
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource()==ok) {
      try {
        ...
        if (nH<1) nH=1;
        if (nH>40) nH=40;
        int nL=Integer.parseInt(w.getText());
        if (nL<1) nL=1;
        if (nL>40) nL=40;
        ...
        minesweep.dispose();
        System.gc();
        MineSweeper minesweepeur = new MineSweeper(nH,nL,nMines,4);
      }
      catch (Exception exc) {
        this.setTitle("Error value");
      }
    }
